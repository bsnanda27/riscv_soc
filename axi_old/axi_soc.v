...
(
    input  wire        clk,
    input  wire        rst,
    input  wire        timer_interrupt, // Preserved at top-level; internal timer is now inside processor
    ...
    //------------------------------------------------------------------
    // Processor <-> AXI Master Intermediary Wires
    //------------------------------------------------------------------
    wire        mem_valid;
    wire        mem_write;
    wire [31:0] mem_addr;
    ...
    //------------------------------------------------------------------
    // New Processor Interface & Debug Monitor Wires
    //------------------------------------------------------------------
    wire [31:0] proc_imem_addr;
    wire [31:0] proc_imem_rdata;
    wire [31:0] pc_debug;
    ...
    // Bridge logic to reconstruct AXI master requests from the new processor ports
    wire data_access = mem_re | mem_we;
    assign mem_valid       = 1'b1;
    assign mem_write       = data_access & mem_we;
    assign mem_addr        = data_access ? proc_mem_addr : proc_imem_addr;
    assign mem_wdata       = mem_we ? proc_mem_wdata : 32'b0;
    assign mem_wstrb       = mem_we ? 4'b1111 : 4'b0000;
    assign proc_imem_rdata = mem_rdata;
    ...
    wire [31:0] gpio_wdata;  wire [3:0] gpio_wstrb;
    wire        gpio_wvalid; wire gpio_wready;
    ...
    (
        ...
        .imem_addr       (proc_imem_addr),
        .imem_rdata      (proc_imem_rdata),
